...
// High-Speed Instant Response & Auto-Answer Middleware for Callback Queries
...
// ================= WEBHOOK SAFETY & SELF-HEALING ENGINE =================
async function checkAndRepairWebhook(reqHost = null, forceRepair = false) {
    try {
        const info = await bot.telegram.getWebhookInfo();
        const host = reqHost || process.env.VERCEL_URL || 'prime-oss.vercel.app';
        const cleanHost = host.replace(/^https?:\/\//, '').replace(/\/.*$/, '');
        const expectedUrl = `https://${cleanHost}/api/bot.js`;

        const isUrlWrong = !info.url || info.url !== expectedUrl;
        const hasRecentErrors = info.last_error_message && info.last_error_date && ((Date.now() / 1000) - info.last_error_date < 86400);

        if (isUrlWrong || hasRecentErrors || forceRepair) {
            console.log(`[Webhook Safety Engine] Auto-repairing Webhook to: ${expectedUrl}`);
            await bot.telegram.setMyCommands([
                { command: 'start', description: 'Start the bot / প্রধান মেনু 🚀' }
            ]).catch(() => {});

            const res = await bot.telegram.setWebhook(expectedUrl, {
            ...
            return { status: 'repaired', url: expectedUrl, success: res, info: updatedInfo };
        }
        return { status: 'healthy', url: info.url, info };
    } catch (err) {
        console.error("[Webhook Safety Engine] Check error:", err.message);
...
async function showAdminManagementMenu(ctx) {
    ...
    const inlineKeyboard = Markup.inlineKeyboard([
        ...
        [
            ...
            Markup.button.callback('🛡️ WEBHOOK SAFETY', 'admin_webhook_safety')
        ],
        [
...
async function ensureWebhookConfigured(hostName) {
    ...
    try {
        ...
        // If webhook is missing, deleted, or points to wrong domain, auto-register!
...
module.exports = async (req, res) => {
    ...
    if (req.method === 'POST') {
        try {
            ...
            // Return 200 OK with error body to keep Telegram webhook healthy & active
